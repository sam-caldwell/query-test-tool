...
// E2E tests that execute the built sqlscore binary and verify that
...
func init() {
	...
	dir, err := os.MkdirTemp("", "sqlscore-e2e")
	if err != nil {
		panic(err)
	}
	e2eBinary = filepath.Join(dir, "sqlscore")
...
// scoreReport is the JSON output structure from sqlscore -format json.
