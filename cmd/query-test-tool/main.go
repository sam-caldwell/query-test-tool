...
var rootCmd = &cobra.Command{
	Use:   "query-test-tool [flags] [SQL]",
	Short: "Score SQL queries for efficiency, memory/compute cost, and cognitive complexity",
	Long: `query-test-tool statically analyzes SQL queries and produces a score predicting
...
func versionTemplate() string {
	return `query-test-tool {{.Version}}
`
}
...
func executeScore(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("version") {
		...
		fmt.Printf("query-test-tool %s (%s)\n", version, commit)
